...
public sealed class ScrapeRuntimeOptions
{
    ...
    public int ChallengeAutoWaitMs { get; set; } = 15000;  // 15 seconds for primary attempts
    /// <summary>How long to wait for challenge auto-resolution on slow retry attempts (ms)</summary>
    public int ChallengeAutoWaitSlowMs { get; set; } = 30000;  // 30 seconds for slow retries
...
public class PlaywrightContextManager : IAsyncDisposable, IDisposable
{
    ...
    private async Task<IBrowserContext> NewIsolatedContextAsync()
    {
        ...
        // Inject Stealth script - critical for bypassing Cloudflare detection
        ...
        try
        {
            if (!string.IsNullOrWhiteSpace(_opt.StealthInitRelativePath))
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    scriptInjected = true;
                    _logger.LogDebug("StealthScript", "Injected stealth script from file", path);
                }
                else
                {
        ...
        {
            _logger.LogWarning("StealthScript", "Failed to inject stealth script from file, will use inline fallback", null, ex);
        }
        
        // Fallback: inject minimal inline stealth script if file loading failed
        if (!scriptInjected)
        {
            try
            {
...
(() => {
    ...
    try { Object.defineProperty(window, 'webdriver', { get: () => undefined }); } catch (e) {}
    try { Object.defineProperty(document, 'webdriver', { get: () => undefined }); } catch (e) {}
...
";
                await ctx.AddInitScriptAsync(inlineScript);
