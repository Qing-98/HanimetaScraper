...
public sealed class PlaywrightNetworkClient : INetworkClient, IAsyncDisposable
{
    ...
    /// <param name="challengeDetector">Detector for identifying challenge pages</param>
    /// <param name="antiBotHook">Optional anti-bot actions to execute on pages</param>
    /// <example>
    /// var options = new PlaywrightClientOptions { ContextTtlMinutes = 30, MaxPagesPerContext = 50 };
    /// var contextManager = new PlaywrightContextManager(browser, logger, options);
        ...
        Func<IPage, CancellationToken, Task>? antiBotHook = null)
    {
        ...
        _browser = contextManager.Browser;
        _opt = contextManager.Options;
        _log = logger;
        ...
        Func<IPage, CancellationToken, Task>? antiBotHook = null)
    {
        ...
        _antiBotHook = antiBotHook;
        _challengeDetector = new ChallengeDetector(Microsoft.Extensions.Logging.Abstractions.NullLogger.Instance);
    ...
    public async Task<JsonDocument> GetJsonAsync(string url, IDictionary<string, string>? headers, CancellationToken ct)
    {
        if (_useContextManager)
        {
            ...
            // Use current UserAgent (updated by manual challenge resolution when needed)
            var userAgent = _opt.UserAgent;
    ...
    private async Task<IPage?> OpenWithRetryAsync(string url, bool forDetail, CancellationToken ct)
    {
        ...
        try
        {
            var page = await OpenOnceAsync(url, forDetail, primary: true, skipManualChallenge: false, ct);
            return page;
        }
        catch (OperationCanceledException) { throw; }
        catch (InvalidOperationException ex) when (ex.Message.Contains("Challenge detected"))
        {
            // Challenge detected - this is the right time to force rotate
            _log?.LogWarning("PageNavigation", "Challenge detected, rotating context", url, ex);
            try
            {
                await _ctxMgr.ForceRotateContextAsync(forDetail);
                _log?.LogDebug("PageNavigation", "Context rotated due to challenge", url);
            }
            catch (Exception rotateEx)
            {
                _log?.LogWarning("ContextRotation", "Failed to rotate context", url, rotateEx);
            }

            // Second attempt with fresh context, skip manual challenge if already attempted
            try
            {
                var page = await OpenOnceAsync(url, forDetail, primary: false, skipManualChallenge: true, ct);
                _log?.LogSuccess("PageNavigation", url);
                _log?.LogDebug("PageNavigation", "Slow retry succeeded after challenge rotation", url);
                return page;
            }
            catch (Exception ex2)
            {
                _log?.LogWarning("PageNavigation", "Slow retry failed after challenge", url, ex2);
                return null;
            }
        }
        catch (Exception ex1)
        {
            // Other errors - only flag for rotation, don't force rotate immediately
            _log?.LogWarning("PageNavigation", "Primary attempt failed", url, ex1);
            
            // Flag context for rotation but continue with slow retry
            ...
            {
                _log?.LogWarning("PageNavigation", "Slow retry also failed", url, ex2);
                return null;
            }
        }
    }
    ...
    private async Task<IPage> OpenOnceAsync(string url, bool forDetail, bool primary, bool skipManualChallenge, CancellationToken ct)
    {
        ...
        try
        {
            await page.GotoAsync(url, new PageGotoOptions
            {
                ...
                Timeout = primary ? _opt.GotoTimeoutMs : _opt.SlowRetryGotoTimeoutMs
            });
            ...
            foreach (var sel in readySelectors)
            {
                try
                {
                    ...
                    {
                        Timeout = primary ? _opt.WaitSelectorTimeoutMs : _opt.SlowRetryWaitSelectorMs
                    });
                }
            ...
            var challengeDetected = await CheckAndWaitForChallengeResolutionAsync(page, url, primary, skipManualChallenge, ct);
    ...
    /// Uses configured URL hints and comprehensive ChallengeDetector for analysis.
    /// </summary>
    ...
    private bool LooksLikeChallenge(string? curUrl, string? html)
    {
        ...
        if (!string.IsNullOrEmpty(curUrl))
        {
            ...
            {
                // More precise URL matching: check if hint is a path segment or query parameter
                if (curUrl.Contains($"/{hint}/", StringComparison.OrdinalIgnoreCase) ||
                    curUrl.Contains($"/{hint}?", StringComparison.OrdinalIgnoreCase) ||
                    curUrl.EndsWith($"/{hint}", StringComparison.OrdinalIgnoreCase) ||
                    curUrl.Contains($"?{hint}=", StringComparison.OrdinalIgnoreCase) ||
                    curUrl.Contains($"&{hint}=", StringComparison.OrdinalIgnoreCase))
                {
                    return true;
        ...
        // 2. Use robust ChallengeDetector for content analysis
        if (!string.IsNullOrEmpty(html))
        {
            if (_challengeDetector.IsCloudflareChallengePage(html, curUrl))
            {
                return true;
            }
        }
    ...
    /// </summary>
    private async Task<bool> CheckAndWaitForChallengeResolutionAsync(IPage page, string url, bool primary, bool skipManualChallenge, CancellationToken ct)
    {
        ...
        // ======= 自动处理失败后尝试手动处理 =======
        if (!skipManualChallenge &&
            _useContextManager && 
            _ctxMgr != null && 
            _ctxMgr.Options.EnableManualChallengeResolution &&
            _manualChallengeHandler != null)
        {
            // 域名级别冷却检查: 防止短时间内重复弹出手动挑战窗口
            var domain = ExtractDomainFromUrl(url);
            if (_manualChallengeAttempts.TryGetValue(domain, out var lastAttempt)
                && DateTime.UtcNow - lastAttempt < ManualChallengeCooldown)
            {
                var elapsed = (DateTime.UtcNow - lastAttempt).TotalSeconds;
                _log?.LogWarning("AntiBot", 
                    $"跳过手动挑战处理 (冷却中, 距上次 {elapsed:F0}秒, 冷却时间 {ManualChallengeCooldown.TotalSeconds}秒)", url);
            }
            else
            {
                _manualChallengeAttempts[domain] = DateTime.UtcNow;
                _log?.LogWarning("AntiBot", "自动处理失败,启动手动挑战处理...", url);
            
                try
                {
                    var result = await _manualChallengeHandler.HandleManualChallengeAsync(
                        url, 
                        _ctxMgr.Options,
                        ct);

                    if (result != null && result.Cookies.Count > 0)
                    {
                        // 关键修复: 更新 ContextManager 的 UserAgent 以匹配手动通过时的浏览器指纹
                        ...
                        {
                            await _ctxMgr.CookieManager.SaveCookiesAsync(domain, result.Cookies); // Save Cookies only
                        }

                        // 我们不尝试在当前已经"脏"了的上下文中继续（因为UA不对）
                        // 而是抛出特定异常，触发 OpenWithRetryAsync 中的上下文轮换
                ...
                {
                    _log?.LogFailure("AntiBot", "手动挑战处理失败", url, ex);
                }
            }
        }
    ...
    /// Ensures DecrementActivePages is always called even if page.CloseAsync() fails.
    /// </summary>
    ...
    private async Task SafeClosePage(IPage? page)
    {
        ...
        // Use try-finally to ensure DecrementActivePages is always called
        try
        {
        ...
        {
            // CRITICAL: Always decrement active page count, even if close fails
            ...
            if (_ctxMgr != null)
            {
                ...
                catch
                {
                    ...
                    _log?.LogDebug("PageCleanup", "Failed to decrement active pages count", null);
                }
            }
        }
    }
    ...
    /// <summary>
    /// 将 cookies 应用到指定的浏览器上下文
    /// </summary>
    private async Task ApplyCookiesToContextAsync(
        ...
        IReadOnlyList<BrowserContextCookiesResult> cookies)
    {
        try
        {
            ...
            _log?.LogSuccess("CookieSync", "Synced to headless context", cookies.Count);
        }
        catch (Exception ex)
        {
            _log?.LogFailure("CookieSync", "同步 cookies 失败", null, ex);
        }
    }
