    ...
    : IRemoteMetadataProvider<Movie, MovieInfo>, IRemoteSearchProvider<MovieInfo>, IHasOrder
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogDebug("No Hanime ID found for {Name}", info.Name);
            return result;
        }

        logger.LogInformation("Fetching metadata for Hanime ID: {Id}", id);
        ...
        logger.LogInformation("Successfully fetched metadata for Hanime ID: {Id}, Title: {Title}", id, result.Item.Name);
        return result;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        // 首先检查是否已经有Hanime ID在ProviderIds中
        var existingId = searchInfo.GetProviderId("Hanime");
        if (!string.IsNullOrWhiteSpace(existingId))
        {
            logger.LogInformation("Found existing Hanime ID in ProviderIds: {Id}", existingId);
        ...
        {
            logger.LogInformation("Detected Hanime ID search: {Id}", hanimeId);
    ...
    private async Task<RemoteSearchResult?> GetDetailByIdAsync(string id, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogError(ex, "Failed to get detail for Hanime ID: {Id}", id);
        }
