...
namespace Jellyfin.Plugin.DLsiteScraper.Client;

/// <summary>
/// HTTP client for communicating with the DLsite scraper backend service.
/// </summary>
public class DLsiteApiClient : IDisposable
{
    ...
    public DLsiteApiClient(ILogger<DLsiteApiClient> logger, string baseUrl, string? apiToken = null, bool enableLogging = true)
    {
        this.logger = logger;
    ...
    /// <summary>
    /// Gets metadata for a specific DLsite ID.
    /// </summary>
    /// <param name="id">The DLsite ID.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The DLsite metadata if found.</returns>
    public async Task<DLsiteMetadata?> GetMetadataAsync(string id, CancellationToken cancellationToken = default)
    {
        try
        {
            LogDebug($"Fetching metadata for DLsite ID: {id}");

            var requestUrl = $"{baseUrl}/api/dlsite/{id}";
    ...
    /// <summary>
    /// Searches for DLsite content by title.
    /// </summary>
    /// <param name="title">The search title.</param>
    /// <param name="maxResults">Maximum number of results to return.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>Collection of search results.</returns>
    public async Task<System.Collections.Generic.IEnumerable<DLsiteSearchResult>> SearchAsync(string title, int maxResults = 10, CancellationToken cancellationToken = default)
    {
        try
        {
            LogDebug($"Searching for title: {title}");

            var url = $"{baseUrl}/api/dlsite/search?title={System.Net.WebUtility.UrlEncode(title)}&max={maxResults}";
            ...
            if (!IsSuccessResponse(apiResponse, out var data))
            {
                ...
                return Array.Empty<DLsiteSearchResult>();
            }
    ...
    private DLsiteMetadata ParseMetadata(JsonElement data, string id)
    {
    ...
    private System.Collections.Generic.IEnumerable<DLsiteSearchResult> ParseSearchResults(JsonElement data)
    {
        var results = new List<DLsiteSearchResult>();
    ...
    private DLsiteSearchResult? ParseSearchResult(JsonElement item)
    {
        ...
        if (enableLogging)
        {
            ...
            {
                LogWarning($"DLsite search result missing primary image: ID={id}, Title={title}");
            }
            else
            {
                LogDebug($"DLsite search result with image: ID={id}, Title={title}, Primary={primary}");
            }
        }

        return new DLsiteSearchResult
        {
            Id = id,
            Title = title,
